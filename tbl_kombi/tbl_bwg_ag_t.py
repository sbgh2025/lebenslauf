...
# Scrollbar für Tätigkeiten
listbox_scroll = tk.Scrollbar(listbox_frame, orient=tk.VERTICAL)
listbox_scroll.pack(side=tk.RIGHT, fill=tk.Y)

# Listbox für Tätigkeiten (Mehrfachauswahl)
listbox_taetigkeiten = tk.Listbox(listbox_frame, selectmode=tk.MULTIPLE, width=70, height=8, yscrollcommand=listbox_scroll.set)
listbox_taetigkeiten.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

# Scrollbar mit Listbox verbinden
...
def refresh_data():
    ...
    # Bewerbung/Arbeitgeber-Zuordnungen laden
    cursor.execute("""
        SELECT ba.bwg_ag_id,
...
tree = ttk.Treeview(tree_frame, columns=("Zuordnung", "Tätigkeit"), show="headings", yscrollcommand=tree_scroll.set, height=15)
...
def add_taetigkeit():
    ...
    for t_id in selected_t_ids:
        cursor.execute("INSERT INTO tbl_bwg_ag_t (bwg_ag_t_bwg_ag_id, bwg_ag_t_t_id) VALUES (?, ?)", (bwg_ag_id, t_id))
...
def delete_entry():
    ...
    if not selected:
        messagebox.showwarning("Fehler", "Bitte eine Tätigkeit auswählen.")
        return
    bwg_ag_t_id = selected[0]
    if messagebox.askyesno("Löschen", "Eintrag wirklich löschen?"):
        cursor.execute("DELETE FROM tbl_bwg_ag_t WHERE bwg_ag_t_id = ?", (bwg_ag_t_id,))
        conn.commit()
        reload_tree()

# Buttons
tk.Button(root, text="Tätigkeiten zuordnen", command=add_taetigkeit).grid(row=2, column=0, padx=10, pady=10, sticky="w")
tk.Button(root, text="Ausgewählte Tätigkeit löschen", command=delete_entry).grid(row=2, column=1, padx=10, pady=10, sticky="e")
