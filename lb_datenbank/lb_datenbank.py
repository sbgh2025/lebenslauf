...
conn = sqlite3.connect("lebenslauf.db")
...
CREATE TABLE IF NOT EXISTS tbl_bwg_ab_swp (
    ...
    FOREIGN KEY (bwg_ab_swp_bwg_ab_id) REFERENCES tbl_bwg_ab(bwg_ab_id) ON DELETE CASCADE,
    FOREIGN KEY (bwg_ab_swp_ab_swp_id) REFERENCES tbl_ab_schwerpunkt(ab_swp_id) ON DELETE CASCADE
);
